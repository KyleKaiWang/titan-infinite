...
void vkglTF::Texture::fromglTfImage(tinygltf::Image& gltfimage, std::string path, Device* device, VkQueue copyQueue)
{
	...
	if (!isKtx) {
		...
		vkCreateBuffer(device->getDevice(), &bufferCreateInfo, nullptr, &stagingBuffer);
		...
		vkAllocateMemory(device->getDevice(), &memAllocInfo, nullptr, &stagingMemory);
		vkBindBufferMemory(device->getDevice(), stagingBuffer, stagingMemory, 0);

		uint8_t* data;
		vkMapMemory(device->getDevice(), stagingMemory, 0, memReqs.size, 0, (void**)&data);
		...
		vkAllocateMemory(device->getDevice(), &memAllocInfo, nullptr, &deviceMemory);
		vkBindImageMemory(device->getDevice(), image, deviceMemory, 0);
		...
		VkCommandBuffer blitCmd = renderer::createCommandBuffer(device->getDevice(), device->getCommandPool(), VK_COMMAND_BUFFER_LEVEL_PRIMARY, true);
		...
		renderer::flushCommandBuffer(device->getDevice(), blitCmd, copyQueue, device->getCommandPool(), true);
	}
	else {
		...
		//VK_CHECK_RESULT(vkCreateImage(device->getDevice(), &imageCreateInfo, nullptr, &image));
		//
		//vkGetImageMemoryRequirements(device->getDevice(), image, &memReqs);
		//memAllocInfo.allocationSize = memReqs.size;
		//memAllocInfo.memoryTypeIndex = device->getMemoryType(memReqs.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
		//VK_CHECK_RESULT(vkAllocateMemory(device->getDevice(), &memAllocInfo, nullptr, &deviceMemory));
		//VK_CHECK_RESULT(vkBindImageMemory(device->getDevice(), image, deviceMemory, 0));
		//
		//VkImageSubresourceRange subresourceRange = {};
		//subresourceRange.aspectMask = VK_IMAGE_ASPECT_COLOR_BIT;
...
void vkglTF::Material::createDescriptorSet(VkDescriptorPool descriptorPool, VkDescriptorSetLayout descriptorSetLayout, uint32_t descriptorBindingFlags)
{
	...
	vkAllocateDescriptorSets(device->getDevice(), &descriptorSetAllocInfo, &descriptorSet);
...
vkglTF::Mesh::Mesh(Device* device, glm::mat4 matrix) {
	...
	buffer::createBuffer(
		...
		&uniformBlock);
	vkMapMemory(device->getDevice(), uniformBuffer.memory, 0, sizeof(uniformBlock), 0, &uniformBuffer.mapped);
...
VertexInputState vkglTF::Vertex::vertexInputState;
...
VertexInputState* vkglTF::Vertex::getVertexInputState(const std::vector<VertexComponent> components) {
	Vertex::vertexInputBindingDescription = Vertex::inputBindingDescription(0);
	Vertex::vertexInputAttributeDescriptions = Vertex::inputAttributeDescriptions(0, components);
	vertexInputState.vertexBindingDescriptions = { Vertex::vertexInputBindingDescription };
	vertexInputState.vertexAttributeDescriptions = Vertex::vertexInputAttributeDescriptions;
...
void vkglTF::Model::createEmptyTexture(VkQueue transferQueue)
{
	...
	imageCreateInfo.usage = VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT;
	vkCreateImage(device->getDevice(), &imageCreateInfo, nullptr, &emptyTexture.image);

	vkGetImageMemoryRequirements(device->getDevice(), emptyTexture.image, &memReqs);
	memAllocInfo.allocationSize = memReqs.size;
	memAllocInfo.memoryTypeIndex = device->findMemoryType(memReqs.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
	vkAllocateMemory(device->getDevice(), &memAllocInfo, nullptr, &emptyTexture.deviceMemory);
	vkBindImageMemory(device->getDevice(), emptyTexture.image, emptyTexture.deviceMemory, 0);
	...
	renderer::flushCommandBuffer(device->getDevice(), copyCmd, transferQueue, device->getCommandPool(), true);
	...
	vkCreateSampler(device->getDevice(), &samplerCreateInfo, nullptr, &emptyTexture.sampler);
	...
	vkCreateImageView(device->getDevice(), &viewCreateInfo, nullptr, &emptyTexture.view);
...
void vkglTF::Model::loadFromFile(std::string filename, Device* device, VkQueue transferQueue, uint32_t fileLoadingFlags, float scale)
{
	...
	if (fileLoaded) {
		...
		throw("Could not load glTF file \"" + filename + "\": " + error, -1);
		return;
	}
	...
	// Descriptors for per-node uniform buffers
	{
		...
		if (descriptorSetLayoutUbo == VK_NULL_HANDLE) {
			descriptorSetLayoutUbo = renderer::createDescriptorSetLayout(
				device->getDevice(),
				{
	...
	// Descriptors for per-material images
	{
		// Layout is global, so only create if it hasn't already been created before
		if (descriptorSetLayoutImage == VK_NULL_HANDLE) {
			descriptorSetLayoutImage = renderer::createDescriptorSetLayout(
				device->getDevice(),
				{
...
void vkglTF::Model::prepareNodeDescriptor(vkglTF::Node* node, VkDescriptorSetLayout descriptorSetLayout) {
	if (node->mesh) {
		...
		descriptorSetAllocInfo.pSetLayouts = &descriptorSetLayout;
		descriptorSetAllocInfo.descriptorSetCount = 1;
		vkAllocateDescriptorSets(device->getDevice(), &descriptorSetAllocInfo, &node->mesh->uniformBuffer.descriptorSet);

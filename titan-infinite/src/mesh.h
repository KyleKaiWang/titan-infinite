...
// To fix std::numeric_limits<float>::max() & min();
#undef max
#undef min
...
namespace vkglTF
{
	...
	*/
	...
	struct Vertex {
		...
		static VertexInputState* getVertexInputState(const std::vector<VertexComponent> components);
	};
	...
	*/
	...
	public:
		...
		void loadFromFile(std::string filename, Device* device, VkQueue transferQueue, uint32_t fileLoadingFlags = vkglTF::FileLoadingFlags::None, float scale = 1.0f);

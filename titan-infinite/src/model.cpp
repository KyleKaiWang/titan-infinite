...
VulkanglTFModel::~VulkanglTFModel()
{
	...
	{
		vkDestroyImageView(device->getDevice(), image.texture.view, nullptr);
		vkDestroyImage(device->getDevice(), image.texture.image, nullptr);
		vkDestroySampler(device->getDevice(), image.texture.sampler, nullptr);
		vkFreeMemory(device->getDevice(), image.texture.imageMemory, nullptr);
	}
	for (Skin skin : skins)
	{
		// TODO : make free resource more reasonable
		vkDestroyBuffer(device->getDevice(), skin.ssbo.resource, nullptr);
	}
}
...
void VulkanglTFModel::loadImages(tinygltf::Model& input)
{
	...
	for (size_t i = 0; i < input.images.size(); i++)
	{
		...
		images[i].texture = texture::loadTexture(buffer, bufferSize, VK_FORMAT_R8G8B8A8_UNORM, glTFImage.width, glTFImage.height, device, copyQueue);
		if (deleteBuffer)
		{
			delete[] buffer;
		}
...
void VulkanglTFModel::loadSkins(tinygltf::Model& input)
{
	...
	for (size_t i = 0; i < input.skins.size(); i++)
	{
		...
		if (glTFSkin.inverseBindMatrices > -1)
		{
			...
			skins[i].ssbo = buffer::createBuffer(
				device,
...
void VulkanglTFModel::updateJoints(VulkanglTFModel::Node* node)
{
	if (node->skin > -1)
	{
		...
		// Update ssbo
		memcpy(skin.ssbo.mapped , jointMatrices.data(), jointMatrices.size() * sizeof(glm::mat4));
	}
